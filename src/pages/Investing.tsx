...
<section className="py-16 md:py-24 bg-gradient-to-br from-background via-accent/5 to-secondary/10">
  <div className="container mx-auto px-4 max-w-6xl">
    ...
    {/* Data (edit freely) */}
    {(() => {
      type Pos = { ticker: string; name?: string; note?: string; tag?: "Core" | "Growth" | "Income" | "Hedge" | "Spec" | "ETF"; };
      const INDIVIDUAL: Pos[] = [
      ...
      const Card3D: React.FC<{ p: Pos }> = ({ p }) => (
        <div
          ...
          onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.transform = "perspective(900px) rotateX(0) rotateY(0) translateY(0)"; }}
        ...
        >
          ...
          <div className="relative">
            ...
            {p.name && <p className="text-sm text-muted-foreground mt-1">{p.name}</p>}
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {p.note && (
                <span className="text-[10px] px-2 py-1 rounded-full bg-secondary/40">
                  {p.note}
                </span>
              )}
            </div>
        ...
        <>
          ...
          <div className="mb-12">
            <div className="flex items-baseline justify-between gap-4 mb-6">
              <h3 className="text-3xl font-bold">Individual <span className="text-primary">Stocks & ETFs</span></h3>
              <span className="text-xs text-muted-foreground">{INDIVIDUAL.length} positions</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {INDIVIDUAL.map((p, i) => <Card3D key={p.ticker + i} p={p} />)}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-baseline justify-between gap-4 mb-6">
              <h3 className="text-3xl font-bold">Roth IRA <span className="text-primary">Holdings</span></h3>
              <span className="text-xs text-muted-foreground">{ROTH_IRA.length} positions</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {ROTH_IRA.map((p, i) => <Card3D key={p.ticker + i} p={p} />)}
            </div>
          </div>
        </>
      );

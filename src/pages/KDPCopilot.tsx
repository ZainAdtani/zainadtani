...
const inputClass = "w-full bg-[#0A0F1A] border border-[#1E3A5F] focus:border-[#447BBE] focus:outline-none rounded-[10px] px-4 py-3 text-[15px] text-white placeholder:text-white/40 transition-colors font-sans";
const labelClass = "block text-[12px] font-bold text-[#E9E4A6] mb-1.5 tracking-widest";
...
Return ONLY valid JSON (no markdown, no backticks, no explanation) in this exact structure:
{
  ...
  return (
    <div className="min-h-screen bg-[#0A0F1A] text-white pb-20">
      ...
      {/* Header */}
      <div className="border-b border-[#1E3A5F] px-6 py-4 flex items-center gap-3">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-8">
        ...
        {error && <p className="text-[#DD5013] text-center text-sm mb-6">{error}</p>}
        ...
        {result && (
          <div className="space-y-5">
            ...
            <div className="bg-[#0E1628] border border-[#447BBE] rounded-2xl p-5">
              <div className="text-[10px] font-bold tracking-[0.2em] text-[#447BBE] mb-4">5 TITLE + SUBTITLE OPTIONS</div>
              {result.titles?.map((t: any, i: number) => (
                <div key={i} className="bg-[#0A0F1A] rounded-lg px-4 py-3 mb-2 flex justify-between items-start gap-3">
                  <div>
                    <div className="font-bold text-[14px]">{t.title}</div>
                    <div className="text-[13px] text-white/50 mt-0.5">{t.subtitle}</div>
                  </div>
            ...
            <div className="bg-[#0E1628] border border-[#D97706] rounded-2xl p-5">
              <div className="text-[10px] font-bold tracking-[0.2em] text-[#D97706] mb-3">AMAZON BOOK DESCRIPTION</div>
              <p className="text-[14px] text-white/80 leading-relaxed mb-3">{result.description}</p>
              <CopyBtn text={result.description} id="desc" />
            </div>

            {/* Keywords */}
            <div className="bg-[#0E1628] border border-[#447BBE] rounded-2xl p-5">
              <div className="text-[10px] font-bold tracking-[0.2em] text-[#447BBE] mb-3">7 KDP KEYWORDS</div>
              <div className="flex flex-wrap gap-2 mb-3">
                {result.keywords?.map((k: string, i: number) => (
                  <span key={i} className="bg-[#0A0F1A] border border-[#1E3A5F] text-[#E9E4A6] text-[12px] px-3 py-1 rounded-lg">{k}</span>
                ))}
              </div>
            ...
            <div className="bg-[#0E1628] border border-[#DD5013] rounded-2xl p-5">
              <div className="text-[10px] font-bold tracking-[0.2em] text-[#DD5013] mb-3">7 AMAZON CATEGORIES</div>
              {result.categories?.map((c: any, i: number) => (
                <div key={i} className="bg-[#0A0F1A] rounded-lg px-4 py-3 mb-2">
                  <div className="font-semibold text-[13px]">{c.path}</div>
                  <div className="text-[12px] text-white/40 mt-0.5">{c.reason}</div>
                </div>
              ))}
            </div>
            ...
            <div className="bg-[#0E1628] border border-[#E9E4A6] rounded-2xl p-5">
              <div className="text-[10px] font-bold tracking-[0.2em] text-[#E9E4A6] mb-3">BACK COVER BLURB</div>
              <p className="text-[14px] text-white/80 leading-relaxed mb-3">{result.backCover}</p>
            ...
            {result.pricingTip && (
              <div className="bg-[#447BBE]/10 border border-[#447BBE] rounded-xl px-5 py-4 flex gap-3 items-start">
                <span>💡</span>
                <span className="text-[14px] text-[#E9E4A6]">{result.pricingTip}</span>
              </div>
            )}
        ...
        {!result && !loading && (
          <div className="text-center py-16 text-white/20">
            ...
            <div className="text-[15px]">Enter your book idea above to get your full KDP launch kit.</div>
          </div>
        )}
      </div>
